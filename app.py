...
if view_key == "el_roll":
    ...
    if table_data:
        ...
        # Simplified mode: hide the edit/delete list
            ...
            if reservations:
                for res in reservations:
                    ...
                    with colC:
                        ...
                        if st.button(f"🗑️ {t('delete')}", key=f"ask_delete_{res_id}"):
                            st.session_state.delete_candidate = res_id
                            st.rerun()

                    # Step 2: confirm UI (survives reruns)
                    if st.session_state.delete_candidate == res_id:
                        st.warning(t("delete_warning"))
                        c1, c2 = st.columns(2)
                        with c1:
                            if st.button(t("confirm_delete"), key=f"confirm_delete_{res_id}", type="primary"):
                                delete_reservation(res_id)
                                st.session_state.delete_candidate = None
                                st.success(t("deleted"))
                                st.rerun()
                        with c2:
                            if st.button(t("cancel"), key=f"cancel_delete_{res_id}"):
                                st.session_state.delete_candidate = None
                                st.rerun()
    ...
    with st.form("reservation_form"):
        ...
        default_room_index = room_options.index(reservation_data[1]) if reservation_data and reservation_data[1] in room_options else 0
        ...
        default_status_index = status_options.index(reservation_data[3]) if reservation_data and reservation_data[3] in status_options else 0
    ...
    if st.session_state.search_query and len(st.session_state.search_query.strip()) >= 2:
        ...
        if reservations:
            ...
            for res in reservations:
                ...
                stay_revenue = float(tariff) * num_nights
                total_revenue += stay_revenue

                table_data.append({
                    t('room'): room_number,
                    t('checkin_date'): check_in,       # date object (better for DateColumn)
                    t('checkout_date'): check_out,     # date object
